...
import { StatusRequest, StatusResponse } from "@/app/api/hacking-pipeline/domain/dtos";
...
export default function AttackStatusDashboard({ pipelineId }: AttackStatusDashboardProps) {
    const [logs, setLogs] = useState<string[]>([]);
    const scrollRef = useRef<HTMLDivElement>(null);
    const [elapsedTime, setElapsedTime] = useState(0);

    const handleStatusSuccess = (data: StatusResponse) => {
        const info = data.pipelineInstanceInfo;
        const timestamp = new Date().toISOString().split('T')[1].slice(0, 8);

        setLogs(prev => {
            const message = `[${timestamp}] > ${info.status} :: ${STATUS_MESSAGES[info.status] || "PROCESSING..."}`;
            const systemMsg = `[${timestamp}] > SYS_CHECK: MEM_OK | CPU_LOAD: ${Math.floor(Math.random() * 30 + 10)}%`;

            const lastLog = prev[prev.length - 1];
            const newLogs = [...prev];

            if (Math.random() > 0.7) {
                newLogs.push(systemMsg);
            }

            if (!lastLog || !lastLog.includes(info.status)) {
    ...
    const { getStatus, data: statusResponse, isError } = useStatusService({
        successCallback: handleStatusSuccess
    });

    // Polling Effect
    useEffect(() => {
        const fetchStatus = () => {
            getStatus(new StatusRequest(pipelineId));
        };
    ...
    return (
        <div className="w-full max-w-5xl p-1 font-mono">
            {/* Main Container with "Hacker" border */}
            <div className="relative bg-black border border-green-500/30 rounded-lg shadow-[0_0_30px_rgba(34,197,94,0.15)] overflow-hidden">
                ...
                <div className="relative flex items-center justify-between p-4 border-b border-green-500/20 bg-green-950/10 backdrop-blur-sm">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-bold tracking-[0.2em] text-green-400 uppercase">
                            CatchMeIfYouHack_Console <span className="text-green-700">{'//'}</span> v1.0
                        </span>
                    </div>
                    <div className="flex items-center gap-4 text-[10px] uppercase tracking-widest text-green-600">
                        <span className="flex items-center gap-2">
                            <Wifi size={12} className={isError ? "text-red-500" : "animate-pulse"} />
                ...
                <div className="relative grid grid-cols-1 md:grid-cols-3 gap-6 p-6">
                    ...
                    <div className="md:col-span-1 space-y-6">
                        ...
                        <div className="bg-zinc-900/50 border border-green-500/20 p-4 rounded">
                            ...
                            <div className="space-y-3 text-xs">
                                <div className="group">
                                    <span className="block text-green-800 text-[10px] uppercase">Target_URL</span>
                                    <span className="text-green-300 break-all">{info?.targetUrl || "LOADING..."}</span>
                                </div>
                                <div>
                                    <span className="block text-green-800 text-[10px] uppercase">Session_ID</span>
                                    <span className="text-green-600 font-mono text-[10px]">{pipelineId.substring(0, 18)}...</span>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="flex flex-col items-center py-4 gap-2">
                                {isComplete ? (
                                    ...
                                    <div className="relative">
                                        ...
                                        <Shield size={48} className="text-green-500 relative z-10 animate-pulse" />
                                    </div>
                                )}
                                <span className={`text-lg font-black tracking-widest uppercase mt-2 ${isFailed ? 'text-red-500' : 'text-green-400'}`}>
                                    {currentStatus}
                                </span>
                            </div>
                        </div>
                    ...
                    <div className="md:col-span-2 flex flex-col h-[400px]">
                        ...
                        >
                            ...
                            {logs.map((log, i) => (
                                <div key={i} className="text-green-500/80 border-l-2 border-green-900/50 pl-2 hover:bg-green-900/10 transition-colors">
                ...
                <div className="p-2 border-t border-green-500/20 bg-black/50 flex justify-between items-center text-[8px] uppercase tracking-[0.2em] text-green-800">
                    <div className="flex gap-4">
                        ...
                        <span className="flex items-center gap-1"><Server size={8} /> NODE_01</span>
                    </div>
                    {isComplete && (
